...
class Solution {
    public int solution(int storey) {
        ...
        while (storey > 0) {
            ...
            if (num == 5) {
                if (storey % 10 >= 5) { // 다음 자릿 수에서 횟수 최소화
                    answer += 10 - num;
                    storey++;
                } else {
                    answer += num;
...
// 마법의 엘리베이터의 버튼을 누르면 현재 층 수에 버튼에 적혀 있는 값을 더한 층으로 이동하게 됩니다. 단, 엘리베이터가 위치해 있는 층과 버튼의 값을 더한 결과가 0보다 작으면 엘리베이터는 움직이지 않습니다. 
...
// 민수와 마법의 엘리베이터가 있는 층을 나타내는 정수 storey 가 주어졌을 때, 0층으로 가기 위해 필요한 마법의 돌의 최소값을 return 하도록 solution 함수를 완성하시오.
